    ...
    var body: some View {
        Form {
            Section(header: Text("Item Name")) {
                // Binding directly to the synced property can cause
                // issues related to the keyboard updating too often. Instead,
                // we'll bind to a `@State` variable and then assign to the
                // managed property when the user presses `Save`
                TextField("New item", text: $itemSummary)
            }
            Section {
                Button(action: {
                    // To avoid updating too many times, we only assign
                    // to the `newItem.summary` once when the user presses `Save`.
